...
def format_date(buf):
    ...
    # Actually: blen = 11
        ...
        if '昨' in buff:
            ...
            realdate = time.strftime('%Y-%m-%d', tstrp_today) + ' ' + date_tail
...
class BaiduTieba(object):
    ...
    def _get_comments(self, ttype, lasttime):                      #ttype: zhuti, reply
        contents = []
        pg = 0
        print("Spider : {}\t{}...".format(self.user, ttype))
        while True:
            pg  += 1
            url = 'https://www.baidu.com/p/sys/data/tieba/feed?rec=1000008&portrait={}&pn={}&type={}&t=1524715347871'.format(self.portrait, pg, ttype)
            # print(url)
            # print("{}, page: {}".format(self.user, pg))
            ...
            if resp:
                ...
                if values:
                    ...
                    if not one_cont:
                        ...
                        break
                    if lasttime:
                        newtime = one_cont[0].get('feed_time')
                        print("Not first Fetch, "+self.user)
                        # print("newtime: " +str(newtime))
                        # print("lasttime: "+str(lasttime))
                        if str(newtime) > str(lasttime):
                            contents.extend(one_cont)
                        else:
                            print("no new {} tie".format(ttype))
                            break
                    else:
                        print("first Fetch, "+self.user)
                        contents.extend(one_cont)
                    ...
                    if hasMore.find('no') != -1:                 # hasMore, None: 可能是首页没有该字段;  hasMore, noMoreFeed:表没有更多内容了
                        print(hasMore)
                        break
            else:
    ...
    def get_topicTie(self, lasttime):
        topic_type = 'zhuti'
        # print("I'm in get_topicTie")
        values = self._get_comments(topic_type, lasttime)
        return values


    def get_replyTie(self, lasttime):
        topic_type = 'reply'
...
def main():
    ...
    if not tryRet:
        return False
    baidu.get_topicTie(lasttime='1500434700')
    # baidu.get_replyTie(lasttime)
